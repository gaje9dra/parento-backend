...
describe.skipIf(!hasDatabase)(
  'Phase 7 monitoring persistence integrity',
  () => {
    ...
    it('prevents older observations from rolling back current state', async () => {
      ...
      const device = await devices.create({
        id: randomUUID(),
        adminId: admin.id,
        stableIdentifier: 'monitor-device',
        name: 'Monitor',
        platform: 'android',
      });
      ...
      await expect(
        monitoring.upsertIfNewer({
          ...
          schemaVersion: 1,
          observedAt: t2,
          receivedAt: new Date('2026-09-25T10:01:01.000Z'),
      ...
      await expect(
        monitoring.upsertIfNewer({
          ...
          schemaVersion: 1,
          observedAt: t1,
          receivedAt: new Date('2026-09-25T10:02:00.000Z'),
    ...
    it('preserves omitted partial fields while allowing explicit null', async () => {
      const admin = await admins.create({
        id: randomUUID(),
        email: 'partial-owner@example.com',
        displayName: null,
      });
      const device = await devices.create({
        id: randomUUID(),
        adminId: admin.id,
        stableIdentifier: 'partial-device',
        name: 'Partial',
        platform: 'android',
      });
      const t1 = new Date('2026-09-25T10:00:00.000Z');
