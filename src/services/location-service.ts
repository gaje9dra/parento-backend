...
export class LocationService {
  ...
  async report(
    ...
    if (!UUID.test(input.reportId))
      throw new AppError(
        400,
        'INVALID_REQUEST',
        'Location report identifier is invalid.',
      );
    if (
      ...
      throw new AppError(
        ...
        'Location observedAt must be a valid timestamp.',
      );
    }
    if (input.observedAt.getTime() > Date.now() + MAX_FUTURE_SKEW_MS) {
      throw new AppError(
        400,
        'INVALID_LOCATION_TIMESTAMP',
        'Location observedAt is too far in the future.',
      );
    }
    ...
    if (
      ...
      throw new AppError(
        ...
        'Managed device is not authorized to report location.',
      );
    }
    ...
    if (input.availability === 'AVAILABLE') {
      if (input.latitude === null || input.longitude === null) {
        throw new AppError(
          ...
          'Latitude and longitude are required when location is available.',
        );
      }
      ...
      if (
        ...
        (!Number.isFinite(input.accuracyMeters) ||
          ...
          input.accuracyMeters > MAX_ACCURACY_METERS)
      ) {
        throw new AppError(
          400,
          'INVALID_LOCATION_ACCURACY',
          'Location accuracy must be a finite non-negative measurement.',
        );
      }
    } else if (
      input.latitude !== null ||
      ...
      throw new AppError(
        400,
        'INVALID_LOCATION_COORDINATES',
        'Unavailable location reports must not include coordinates.',
      );
    }
  ...
  private validateCoordinate(
    ...
    if (
      ...
      value < min ||
      value > max ||
      decimalPlaces(value) > MAX_COORDINATE_DECIMAL_PLACES
    ) {
      throw new AppError(
        400,
        'INVALID_LOCATION_COORDINATES',
        label + ' is outside the permitted geographic range or precision.',
      );
    }
  }
}
