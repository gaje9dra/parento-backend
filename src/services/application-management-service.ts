...
export class ApplicationManagementService {
  ...
  async submitInventory(
    ...
    if (input.schemaVersion !== 1)
      throw new AppError(
        ...
        'Unsupported application inventory schema.',
      );
    if (!UUID.test(input.synchronizationId))
      throw new AppError(
        400,
        'INVALID_REQUEST',
        'Inventory synchronization identifier is invalid.',
      );
    if (
      input.observedAt.getTime() >
        Date.now() + this.options.maxFutureSkewSeconds * 1000 ||
      !Number.isFinite(input.observedAt.getTime()) ||
      input.observedAt.getTime() < 0
    )
      throw new AppError(
        400,
        'INVALID_APPLICATION_TIMESTAMP',
        'Application inventory timestamp is invalid.',
      );
    if (input.applications.length > this.options.maxInventoryItems)
      throw new AppError(
        413,
        'APPLICATION_INVENTORY_TOO_LARGE',
        'Application inventory exceeds the configured application-count limit.',
      );
    ...
    for (const item of input.applications) {
      ...
      if (seen.has(item.packageName))
        throw new AppError(
          400,
          'DUPLICATE_APPLICATION_PACKAGE',
          'Application inventory contains a duplicate package name.',
        );
    ...
    if (payloadBytes > this.options.maxInventoryPayloadBytes)
      throw new AppError(
        413,
        'APPLICATION_INVENTORY_TOO_LARGE',
        'Application inventory exceeds the configured payload limit.',
      );
    ...
    if (
      ...
      throw new AppError(
        ...
        'Managed-device application inventory is not authorized.',
      );
    ...
    logger.info(
      {
        event: 'application_inventory_synchronized',
        managedDeviceId: session.managedDeviceId,
        accepted: result.applied,
      },
      'Application inventory synchronized',
    );
    return result;
  }
  ...
  async getInventoryItem(
    ...
    const item = await this.repository.findInventoryItem(
      device.id,
      packageName,
    );
    if (!item)
      throw new AppError(
        ...
        'Application inventory item was not found.',
      );
    return item;
  }
  ...
  async createPolicy(
    ...
    logger.info(
      {
        event: 'application_policy_created',
        adminId,
        policyId: policy.id,
  ...
  async updatePolicy(
    ...
    if (!UUID.test(policyId))
      throw new AppError(
        400,
        'INVALID_POLICY_ID',
        'Application policy identifier is invalid.',
      );
    this.validateRules(input.rules);
    ...
    if (!policy)
      throw new AppError(
        404,
        'APPLICATION_POLICY_NOT_FOUND',
        'Application policy was not found.',
      );
    const assignments = await this.repository.listAssignmentsForPolicy(
      policy.id,
      adminId,
    );
    await Promise.all(
      assignments.map((assignment) =>
        this.enqueuePolicySync(
          adminId,
          assignment.managedDeviceId,
          policy.id,
          policy.version,
        ),
      ),
    ...
    logger.info(
      {
        event: 'application_policy_updated',
        adminId,
        policyId: policy.id,
        version: policy.version,
      },
  ...
  async disablePolicy(adminId: string, policyId: string) {
    ...
    const policy = await this.repository.disablePolicy(
      policyId,
      adminId,
      adminId,
    );
    ...
    logger.info(
      {
        event: 'application_policy_disabled',
        adminId,
        policyId: policy.id,
        version: policy.version,
      },
  ...
  async assignPolicy(adminId: string, deviceId: string, policyId: string) {
    ...
    if (policy.status !== 'ACTIVE')
      throw new AppError(
        409,
        'APPLICATION_POLICY_DISABLED',
        'Only an active application policy can be assigned.',
      );
    const device = await this.requireOwnedDevice(adminId, deviceId);
    ...
    logger.info(
      {
        event: 'application_policy_assigned',
        adminId,
        managedDeviceId: device.id,
        policyId: policy.id,
        policyVersion: policy.version,
      },
      'Application policy assigned',
    );
    return assignment;
  }
  ...
  async removeAssignment(adminId: string, deviceId: string) {
    ...
    if (!removed)
      throw new AppError(
        404,
        'APPLICATION_POLICY_ASSIGNMENT_NOT_FOUND',
        'Application policy assignment was not found.',
      );
    logger.info(
      {
        event: 'application_policy_assignment_removed',
        adminId,
        managedDeviceId: deviceId,
      },
      'Application policy assignment removed',
    );
    return { removed: true };
  }
  ...
  async reportEnforcement(
    ...
    if (
      ...
      throw new AppError(
        ...
        'Application policy version is invalid.',
      );
    if ((input.policyId === null) !== (input.policyVersion === null))
      throw new AppError(
        400,
        'INVALID_REQUEST',
        'Reported policy identifier and version must be provided together.',
      );
    const device = await this.devices.findById(session.managedDeviceId);
    if (
      device === null ||
      ...
      throw new AppError(
        403,
        'DEVICE_AUTHORIZATION_DENIED',
        'Managed-device application enforcement reporting is not authorized.',
      );

    const current = await this.repository.getEnforcementState(
      device.id,
      device.adminId,
    );
    if (!current)
      throw new AppError(
        409,
        'APPLICATION_POLICY_STATE_UNAVAILABLE',
        'Application policy state is not initialized.',
      );
    if (
      input.status === 'APPLIED' &&
      (input.policyId !== current.desiredPolicyId ||
        input.policyVersion !== current.desiredPolicyVersion)
    )
      throw new AppError(
        409,
        'STALE_APPLICATION_POLICY',
        'Reported applied policy is not the current desired policy.',
      );
    ...
    logger.info(
      {
        event: 'application_enforcement_status_changed',
        managedDeviceId: device.id,
        status: result.status,
      },
      'Application enforcement status changed',
    );
    return result;
  }
  ...
  async requestInventory(adminId: string, deviceId: string) {
    ...
    if (
      ...
      throw new AppError(
        409,
        'DEVICE_NOT_READY',
        'Managed device is not available for application inventory requests.',
      );
  ...
  private async enqueuePolicySync(
    adminId: string,
    deviceId: string,
    policyId: string,
    ...
    try {
      ...
      if (error instanceof AppError && error.code === 'DEVICE_NOT_READY')
        throw error;
      return null;
    }
  }
  ...
  private validateRules(
    ...
    if (rules.length > this.options.maxRuleCount)
      throw new AppError(
        413,
        'APPLICATION_POLICY_TOO_LARGE',
        'Application policy exceeds the configured rule-count limit.',
      );
    const seen = new Set<string>();
    for (const rule of rules) {
      ...
      if (seen.has(rule.packageName))
        throw new AppError(
          400,
          'DUPLICATE_APPLICATION_RULE',
          'Application policy contains a duplicate package rule.',
        );
      ...
      if (rule.action !== 'ALLOW' && rule.action !== 'BLOCK')
        throw new AppError(
          ...
          'Application policy action is invalid.',
        );
    }
  }
