...
export class CommandService {
  ...
  async create(input: {
    ...
    if (input.idempotencyKey.length < 8 || input.idempotencyKey.length > 128 || !/^[A-Za-z0-9._:-]+$/.test(input.idempotencyKey)) {
      throw new AppError(400, 'INVALID_REQUEST', 'Idempotency key has an invalid format.');
    }

    const existing = await this.repository.findByIdempotency(input.adminId, input.idempotencyKey);
    if (existing !== null) return existing;

    const now = new Date();
    const expiresAt = input.expiresAt ?? new Date(now.getTime() + this.options.defaultTtlSeconds * 1000);
    if (expiresAt.getTime() <= now.getTime() || expiresAt.getTime() > now.getTime() + this.options.maxTtlSeconds * 1000) {
    ...
    try {
      ...
      const queued = await this.repository.transition(command.id, 'CREATED', 'QUEUED', now);
      if (queued === null) throw new AppError(500, 'INTERNAL_SERVER_ERROR', 'Command could not be queued.');
      const delivery = await this.delivery.deliver(queued);
      if (delivery.delivered) {
        return (await this.repository.transition(queued.id, 'QUEUED', 'DELIVERING', new Date(), {
      ...
      if (error instanceof Error && 'code' in error && (error as { code?: string }).code === 'CONFLICT') {
        const duplicate = await this.repository.findByIdempotency(input.adminId, input.idempotencyKey);
  ...
  async getOwned(id: string, adminId: string): Promise<DeviceCommand> {
    if (!UUID_PATTERN.test(id)) throw new AppError(400, 'INVALID_REQUEST', 'Command identifier is invalid.');
  ...
  async cancel(id: string, adminId: string): Promise<DeviceCommand> {
    ...
    if (!['CREATED', 'QUEUED'].includes(current.status)) {
      throw new AppError(409, 'CONFLICT', 'Command cannot be cancelled in its current state.');
    }
    const result = await this.repository.transition(id, current.status, 'CANCELLED', new Date());
    if (result === null) throw new AppError(404, 'RESOURCE_NOT_FOUND', 'Command was not found.');
    return result;
  }
  ...
  async transitionFromDevice(
    ...
    if (command.sessionId !== null && command.sessionId !== sessionId) {
      throw new AppError(403, 'AUTHORIZATION_DENIED', 'Command session is not authorized.');
    }
