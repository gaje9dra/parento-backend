import { randomUUID } from 'node:crypto';
import type { DeviceConnectionSession } from '../domain/device-connection-session.js';
import type {
  AudioAccessSession,
  AudioAccessSessionStatus,
} from '../domain/audio-access-session.js';
import type { ManagedDeviceRepository } from '../repositories/managed-device-repository.js';
import type { DeviceConnectionSessionRepository } from '../repositories/device-connection-session-repository.js';
import type { AudioAccessSessionRepository } from '../repositories/audio-access-session-repository.js';
import type { CommandService } from './command-service.js';
import { PersistenceError } from '../domain/persistence-errors.js';
import { AppError } from '../types/errors.js';

export interface AudioAccessServiceOptions {
  readonly maxDurationSeconds: number;
  readonly retentionSeconds?: number | undefined;
}

const UUID =
  /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;

const sanitizeTransportState = (
  value: Record<string, unknown> | null,
): Record<string, string> | null => {
  if (value === null) return null;

  const allowed = new Set(['state', 'transport', 'connectionId']);
  const allowedStates = new Set([
    'AUTHORIZATION_PENDING',
    'AUTHORIZED',
    'STARTING',
    'ACTIVE',
    'STOPPING',
    'STOPPED',
    'EXPIRED',
    'FAILED',
    'DISCONNECTED',
  ]);
  const result: Record<string, string> = {};

  for (const [key, raw] of Object.entries(value)) {
    if (!allowed.has(key) || typeof raw !== 'string' || raw.length > 128) {
      throw new AppError(
        400,
        'INVALID_REQUEST',
        'Audio transport state contains unsupported or invalid metadata.',
      );
    }
    if (key === 'state' && !allowedStates.has(raw)) {
      throw new AppError(
        400,
        'INVALID_REQUEST',
        'Audio transport state contains an unsupported lifecycle state.',
      );
    }
    result[key] = raw;
  }

  return result;
};

export class AudioAccessService {
  constructor(
    private readonly sessions: AudioAccessSessionRepository,
    private readonly devices: ManagedDeviceRepository,
    private readonly deviceSessions: DeviceConnectionSessionRepository,
    private readonly commands: CommandService,
    private readonly options: AudioAccessServiceOptions,
  ) {}

  async request(
    adminId: string,
    deviceId: string,
    correlationId?: string | null,
  ): Promise<{ session: AudioAccessSession; created: boolean }> {
    if (!UUID.test(deviceId)) {
      throw new AppError(
        400,
        'INVALID_REQUEST',
        'Managed-device identifier is invalid.',
      );
    }

    const device = await this.devices.findById(deviceId);
    if (device === null) {
      throw new AppError(
        404,
        'DEVICE_NOT_FOUND',
        'Managed device was not found.',
      );
    }
    if (device.adminId !== adminId) {
      throw new AppError(
        403,
        'AUTHORIZATION_DENIED',
        'The administrator does not control this device.',
      );
    }
    if (
      device.enrollmentStatus !== 'ACTIVE' ||
      device.operationalStatus !== 'ACTIVE'
    ) {
      throw new AppError(
        409,
        'DEVICE_NOT_READY',
        'Managed device is not authorized for audio access.',
      );
    }

    const connection =
      (await this.deviceSessions.findActiveByDeviceId?.(deviceId)) ?? null;
    if (
      connection === null ||
      connection.state !== 'CONNECTED' ||
      connection.expiresAt.getTime() <= Date.now()
    ) {
      throw new AppError(
        409,
        'DEVICE_UNAVAILABLE',
        'Managed device does not have a valid communication session.',
      );
    }

    await this.expireDue();
    await this.sessions.deleteTerminatedBefore(
      new Date(Date.now() - (this.options.retentionSeconds ?? 2592000) * 1000),
      100,
    );

    const now = new Date();
    const result = await this.sessions.create({
      id: randomUUID(),
      managedDeviceId: deviceId,
      adminId,
      correlationId:
        correlationId && /^[A-Za-z0-9._:-]{1,128}$/.test(correlationId)
          ? correlationId
          : randomUUID(),
      expiresAt: new Date(
        now.getTime() + this.options.maxDurationSeconds * 1000,
      ),
      transportState: { state: 'AUTHORIZATION_PENDING' },
    });

    if (!result.created) {
      if (
        result.session.adminId === adminId &&
        correlationId !== null &&
        correlationId !== undefined &&
        result.session.correlationId === correlationId
      ) {
        return { session: result.session, created: false };
      }
      throw new AppError(
        409,
        'AUDIO_SESSION_ALREADY_ACTIVE',
        'An audio-access session is already active or starting for this device.',
      );
    }

    let session = result.session;
    try {
      session = await this.transition(session, 'AUTHORIZED', null, {
        state: 'AUTHORIZED',
      });
      const command = await this.commands.createAudioAccessCommand(adminId, {
        deviceId,
        type: 'START_AUDIO_ACCESS',
        audioSessionId: session.id,
        correlationId: session.correlationId,
      });
      session = await this.transition(session, 'STARTING', null, {
        state: 'STARTING',
        startCommandId: command.command.id,
      });
      return { session, created: true };
    } catch (error) {
      await this.failBestEffort(session);
      throw error;
    }
  }

  async getOwned(id: string, adminId: string): Promise<AudioAccessSession> {
    const session = await this.sessions.findOwned(id, adminId);
    if (session === null) {
      throw new AppError(
        404,
        'AUDIO_SESSION_NOT_FOUND',
        'Audio-access session was not found.',
      );
    }
    if (
      session.expiresAt.getTime() <= Date.now() &&
      !this.isTerminal(session.status)
    ) {
      return this.expire(session);
    }
    return session;
  }

  async stop(id: string, adminId: string): Promise<AudioAccessSession> {
    let session = await this.getOwned(id, adminId);
    if (this.isTerminal(session.status)) return session;
    if (session.status === 'STOPPING') return session;

    session = await this.transition(session, 'STOPPING', 'ADMIN_STOP', {
      state: 'STOPPING',
    });

    try {
      await this.commands.createAudioAccessCommand(adminId, {
        deviceId: session.managedDeviceId,
        type: 'STOP_AUDIO_ACCESS',
        audioSessionId: session.id,
        correlationId: session.correlationId,
      });
      return session;
    } catch (error) {
      await this.failBestEffort(session);
      throw error;
    }
  }

  async markStarted(
    sessionId: string,
    deviceSession: Pick<
      DeviceConnectionSession,
      'managedDeviceId' | 'state' | 'expiresAt'
    >,
    transportState: Record<string, unknown> | null,
  ): Promise<AudioAccessSession> {
    this.assertConnectedDeviceSession(deviceSession);
    const session = await this.sessions.findById(sessionId);
    if (session === null) {
      throw new AppError(
        404,
        'AUDIO_SESSION_NOT_FOUND',
        'Audio-access session was not found.',
      );
    }
    if (session.managedDeviceId !== deviceSession.managedDeviceId) {
      throw new AppError(
        403,
        'AUTHORIZATION_DENIED',
        'The audio-access session is not assigned to this device.',
      );
    }
    if (session.expiresAt.getTime() <= Date.now()) return this.expire(session);
    if (session.status !== 'STARTING') {
      throw new AppError(
        409,
        'AUDIO_SESSION_STATE_CONFLICT',
        'The audio-access session is not starting.',
      );
    }

    return this.transition(
      session,
      'ACTIVE',
      null,
      sanitizeTransportState(transportState),
    );
  }

  async markStopped(
    sessionId: string,
    deviceSession: Pick<
      DeviceConnectionSession,
      'managedDeviceId' | 'state' | 'expiresAt'
    >,
  ): Promise<AudioAccessSession> {
    this.assertConnectedDeviceSession(deviceSession);
    const session = await this.sessions.findById(sessionId);
    if (session === null) {
      throw new AppError(
        404,
        'AUDIO_SESSION_NOT_FOUND',
        'Audio-access session was not found.',
      );
    }
    if (session.managedDeviceId !== deviceSession.managedDeviceId) {
      throw new AppError(
        403,
        'AUTHORIZATION_DENIED',
        'The audio-access session is not assigned to this device.',
      );
    }
    if (this.isTerminal(session.status)) return session;
    if (session.status !== 'STOPPING') {
      throw new AppError(
        409,
        'AUDIO_SESSION_STATE_CONFLICT',
        'The audio-access session cannot be stopped from its current state.',
      );
    }

    return this.transition(session, 'STOPPED', 'ADMIN_STOP', {
      state: 'STOPPED',
    });
  }

  async expireDue(): Promise<number> {
    return this.sessions.expireDue(new Date(), 100);
  }

  private async expire(
    session: AudioAccessSession,
  ): Promise<AudioAccessSession> {
    if (this.isTerminal(session.status)) return session;

    try {
      if (
        ['AUTHORIZED', 'STARTING', 'ACTIVE', 'STOPPING'].includes(
          session.status,
        )
      ) {
        await this.commands.createAudioAccessCommand(session.adminId, {
          deviceId: session.managedDeviceId,
          type: 'STOP_AUDIO_ACCESS',
          audioSessionId: session.id,
          correlationId: session.correlationId,
        });
      }
      return await this.sessions.transition({
        id: session.id,
        from: session.status,
        to: 'EXPIRED',
        now: new Date(),
        terminationReason: 'EXPIRED',
        transportState: { state: 'EXPIRED' },
      });
    } catch (error) {
      if (error instanceof PersistenceError) {
        const latest = await this.sessions.findById(session.id);
        if (latest !== null) return latest;
      }
      throw error;
    }
  }

  private async transition(
    session: AudioAccessSession,
    to: AudioAccessSessionStatus,
    terminationReason: AudioAccessSession['terminationReason'],
    transportState: Record<string, unknown> | null,
  ): Promise<AudioAccessSession> {
    try {
      return await this.sessions.transition({
        id: session.id,
        from: session.status,
        to,
        now: new Date(),
        terminationReason,
        transportState,
      });
    } catch (error) {
      if (error instanceof PersistenceError) {
        throw new AppError(409, 'AUDIO_SESSION_STATE_CONFLICT', error.message);
      }
      throw error;
    }
  }

  private async failBestEffort(session: AudioAccessSession): Promise<void> {
    if (this.isTerminal(session.status)) return;
    try {
      await this.sessions.transition({
        id: session.id,
        from: session.status,
        to: 'FAILED',
        now: new Date(),
        terminationReason: 'FAILED',
        transportState: { state: 'FAILED' },
      });
    } catch {
      // Preserve the original failure; database state remains authoritative.
    }
  }

  private assertConnectedDeviceSession(
    session: Pick<DeviceConnectionSession, 'state' | 'expiresAt'>,
  ): void {
    if (
      session.expiresAt.getTime() <= Date.now() ||
      session.state !== 'CONNECTED'
    ) {
      throw new AppError(
        401,
        'DEVICE_SESSION_INVALID',
        'Managed-device session is no longer valid.',
      );
    }
  }

  private isTerminal(status: AudioAccessSessionStatus): boolean {
    return ['STOPPED', 'EXPIRED', 'FAILED', 'REJECTED'].includes(status);
  }
}
