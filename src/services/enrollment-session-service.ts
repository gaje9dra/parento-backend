  ...
  async consume(input: {
    ...
    try {
      ...
      if (
        ...
        error.message === 'Enrollment session not found.'
      ) {
        throw new AppError(
          404,
          'ENROLLMENT_NOT_FOUND',
      ...
      if (error instanceof Error && error.message === 'Enrollment session expired.') {
        throw new AppError(
          410,
          'ENROLLMENT_EXPIRED',
          'Enrollment session has expired.',
      ...
      if (
        error instanceof Error &&
        error.message === 'Enrollment verification is no longer available.'
      ) {
        throw new AppError(
          429,
          'RATE_LIMITED',
      ...
      if (
        error instanceof Error &&
        error.message === 'Enrollment verification failed.'
      ) {
        throw new AppError(
          401,
          'INVALID_ENROLLMENT_VERIFICATION',
      ...
      if (
        error instanceof Error &&
        error.message === 'Enrollment authorization is no longer available.'
      ) {
        throw new AppError(
          403,
