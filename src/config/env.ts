...
const rawEnvSchema = z.object({
  ...
  DEVICE_SESSION_TTL_SECONDS: z.coerce.number().int().min(60).max(86400).default(900),
  COMMAND_TTL_SECONDS: z.coerce.number().int().min(10).max(86400).default(300),
  COMMAND_MAX_TTL_SECONDS: z.coerce.number().int().min(10).max(604800).default(3600),
...
const productionRequirements = (env: Record<string, unknown>) => {
  ...
  if (env.NODE_ENV === 'production') {
    ...
    if (typeof env.CORS_ORIGINS !== 'string' || env.CORS_ORIGINS.trim().length === 0) {
      issues.push({ path: ['CORS_ORIGINS'], message: 'Required in production.' });
    }
    if (env.RATE_LIMIT_ENABLED !== true) {
      issues.push({ path: ['RATE_LIMIT_ENABLED'], message: 'Authentication rate limiting must be enabled in production.' });
    }
  }
  return issues;
};
...
export interface AppConfig {
  ...
  readonly database: { readonly url?: string; readonly poolMax: number; readonly idleTimeoutMs: number; readonly connectionTimeoutMs: number; readonly ssl: boolean };
  ...
  readonly security: {
    ...
    readonly commandMaxTtlSeconds: number;
...
const parseOrigins = (value: string): string[] => {
  ...
  for (const origin of origins) {
    ...
    try { parsed = new URL(origin); } catch {
      throw new ConfigurationError([{ variable: 'CORS_ORIGINS', message: 'Origins must be valid HTTP(S) origins.' }]);
    }
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      throw new ConfigurationError([{ variable: 'CORS_ORIGINS', message: 'Origins must use HTTP or HTTPS.' }]);
    }
    if (parsed.pathname !== '/' || parsed.search !== '' || parsed.hash !== '' || parsed.username !== '' || parsed.password !== '') {
      throw new ConfigurationError([{ variable: 'CORS_ORIGINS', message: 'Origins must contain only scheme, host, and optional port.' }]);
    }
  }
  return origins;
};
...
const validateRequestBodyLimit = (value: string): void => {
  ...
  if (match === null) throw new ConfigurationError([{ variable: 'REQUEST_BODY_LIMIT', message: 'Expected a size such as 100kb or 1mb.' }]);
  const amountText = match[1];
  const unitText = match[2];
  if (amountText === undefined || unitText === undefined) throw new ConfigurationError([{ variable: 'REQUEST_BODY_LIMIT', message: 'Expected a size such as 100kb or 1mb.' }]);
  const amount = Number(amountText);
  const multiplier = unitText.toLowerCase() === 'b' ? 1 : unitText.toLowerCase() === 'kb' ? 1024 : unitText.toLowerCase() === 'mb' ? 1024 ** 2 : 1024 ** 3;
  if (!Number.isFinite(amount) || amount * multiplier > 10 * 1024 * 1024) throw new ConfigurationError([{ variable: 'REQUEST_BODY_LIMIT', message: 'Request body limit must not exceed 10mb.' }]);
};
...
export function loadConfig(env: NodeJS.ProcessEnv = process.env): AppConfig {
  if (env.NODE_ENV === 'production' && (env.CORS_ORIGINS === undefined || env.CORS_ORIGINS.trim() === '')) {
    throw new ConfigurationError([{ variable: 'CORS_ORIGINS', message: 'Required in production configuration.' }]);
  }
  const parsed = rawEnvSchema.safeParse(env);
  if (!parsed.success) {
    throw new ConfigurationError(parsed.error.issues.map((issue) => ({ variable: issue.path.join('.') || 'environment', message: issue.message })));
  }
  if (parsed.data.DATABASE_URL !== undefined) {
    const databaseUrl = new URL(parsed.data.DATABASE_URL);
    if (databaseUrl.protocol !== 'postgres:' && databaseUrl.protocol !== 'postgresql:') {
      throw new ConfigurationError([{ variable: 'DATABASE_URL', message: 'Database URL must use PostgreSQL.' }]);
    }
  }
  if (parsed.data.AUTH_ACCESS_TOKEN_TTL_SECONDS > parsed.data.SESSION_TTL_SECONDS) {
    throw new ConfigurationError([{ variable: 'AUTH_ACCESS_TOKEN_TTL_SECONDS', message: 'Access-token lifetime cannot exceed session lifetime.' }]);
  }
  if (parsed.data.COMMAND_TTL_SECONDS > parsed.data.COMMAND_MAX_TTL_SECONDS) {
    throw new ConfigurationError([{ variable: 'COMMAND_TTL_SECONDS', message: 'Command default TTL cannot exceed maximum TTL.' }]);
  }
  if (parsed.data.DEVICE_SESSION_TTL_SECONDS > parsed.data.COMMAND_MAX_TTL_SECONDS) {
    throw new ConfigurationError([{ variable: 'DEVICE_SESSION_TTL_SECONDS', message: 'Device session TTL cannot exceed command maximum TTL.' }]);
  }
  ...
  if (parsed.data.CORS_CREDENTIALS && corsOrigins.includes('*')) {
    throw new ConfigurationError([{ variable: 'CORS_CREDENTIALS', message: 'Credentials cannot be enabled with a wildcard CORS origin.' }]);
  }
  if (parsed.data.NODE_ENV === 'production') {
    if (corsOrigins.length === 0) throw new ConfigurationError([{ variable: 'CORS_ORIGINS', message: 'At least one explicit origin is required in production.' }]);
    if (corsOrigins.includes('*')) throw new ConfigurationError([{ variable: 'CORS_ORIGINS', message: 'Wildcard origin is not allowed in production.' }]);
  }
  return {
    ...
    database: {
      ...(parsed.data.DATABASE_URL === undefined ? {} : { url: parsed.data.DATABASE_URL }),
    ...
    security: {
      ...
      commandMaxTtlSeconds: parsed.data.COMMAND_MAX_TTL_SECONDS,
