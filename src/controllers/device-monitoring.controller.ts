...
const managementMode = z.enum([
  'UNMANAGED',
  'PROFILE_OWNER',
  'DEVICE_OWNER',
  'UNKNOWN',
]);
const networkState = z.enum([
  'UNKNOWN',
  'OFFLINE',
  'WIFI',
  'CELLULAR',
  ...
  state === null
    ...
    : {
        ...
        lastSuccessfulInitializationAt:
          state.lastSuccessfulInitializationAt?.toISOString() ?? null,
        lastSuccessfulCommunicationAt:
          state.lastSuccessfulCommunicationAt?.toISOString() ?? null,
...
export const createDeviceMonitoringController = (
  ...
  list: (async (req, res, next) => {
    ...
    try {
      const page = await service.listForAdmin(admin.id, {
        limit,
        cursor: cursorRaw,
      });
      res.status(200).json({
        data: {
          devices: page.items.map((item) => ({
...
export const monitoringContentLengthLimit =
