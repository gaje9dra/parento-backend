...
export const createAudioAccessController = (service: AudioAccessService) => ({
      ...
      if (!p.success || !b.success) {
        res.status(400).json({
          error: {
            ...
            message: 'Invalid audio-access request.',
          },
    ...
    try {
      ...
      if (!p.success) {
        res.status(400).json({
          error: {
            code: 'INVALID_REQUEST',
            message: 'Invalid audio session identifier.',
          },
          requestId: res.locals.requestId,
        });
        return;
    ...
    try {
      ...
      if (!p.success || !b.success) {
        res.status(400).json({
          error: {
            code: 'INVALID_REQUEST',
            message: 'Invalid audio session state update.',
          },
          requestId: res.locals.requestId,
        });
        return;
