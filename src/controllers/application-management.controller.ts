...
const rule = z
  .object({ packageName, action: z.enum(['ALLOW', 'BLOCK']) })
  .strict();
...
export const createApplicationManagementController = (
    ...
    if (!parsed.success) {
      res.status(400).json({
        error: {
          ...
          message: 'Invalid application inventory payload.',
        },
        requestId: res.locals.requestId,
      });
      return;
  ...
  getDevicePolicy: (async (req, res, next) => {
    ...
    try {
      const result = await service.getDevicePolicy(
        req.authenticatedDeviceSession.managedDeviceId,
      );
  ...
  reportDeviceStatus: (async (req, res, next) => {
    ...
    if (!parsed.success) {
      res.status(400).json({
        error: {
          code: 'INVALID_REQUEST',
          message: 'Invalid application policy status payload.',
        },
        requestId: res.locals.requestId,
      });
      return;
  ...
  getInventoryItem: (async (req, res, next) => {
    ...
    try {
      const item = await service.getInventoryItem(
        req.authenticatedAdmin.id,
        p.data.deviceId,
        p.data.packageName,
      );
    ...
    try {
      ...
      if (
        ...
        res.status(400).json({
          error: {
            code: 'INVALID_REQUEST',
            message: 'Application policy page limit must be between 1 and 100.',
          },
          requestId: res.locals.requestId,
        });
        return;
  ...
  removePolicy: (async (req, res, next) => {
    ...
    try {
      const result = await service.removePolicy(
        req.authenticatedAdmin.id,
        p.data.deviceId,
        b.data.policyId,
      );
...
===== src/domain/application-management.ts =====
...
export const isValidAndroidPackageName = (value: string): boolean =>
  ...
  value.length <= 255;
===== src/repositories/application-inventory-repository.ts =====
...
===== src/repositories/application-policy-repository.ts =====
import type {
  ApplicationPolicy,
...
export interface ApplicationPolicyRepository extends Repository {
  ...
  findSyncState(
    managedDeviceId: string,
  ): Promise<ApplicationPolicySyncState | null>;
...
===== src/repositories/postgres-application-inventory-repository.ts =====
