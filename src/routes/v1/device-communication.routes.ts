...
export const createDeviceCommunicationRouter=(communication:DeviceCommunicationService,commands:CommandService,credentials:DeviceCredentialRepository,sessions:DeviceConnectionSessionRepository,rateLimitConfig:AppConfig['rateLimit']):Router=>{
 const router=Router(); const controller=createDeviceCommunicationController(communication,commands); const limiter=createDeviceCommunicationRateLimiter({enabled:rateLimitConfig.enabled,windowMs:rateLimitConfig.windowMs,maxRequests:rateLimitConfig.maxRequests});
 const limited=limiter===undefined?[]:[limiter];
 router.post('/device/sessions',...limited,requireDeviceCredential(credentials),controller.connect);
 router.post('/device/sessions/heartbeat',...limited,requireDeviceSession(sessions),controller.heartbeat);
 router.post('/device/sessions/disconnect',...limited,requireDeviceSession(sessions),controller.disconnect);
 router.post('/device/commands/:commandId/ack',...limited,requireDeviceSession(sessions),controller.acknowledge);
 router.post('/device/commands/:commandId/start',...limited,requireDeviceSession(sessions),controller.start);
 router.post('/device/commands/:commandId/result',...limited,requireDeviceSession(sessions),controller.result);
