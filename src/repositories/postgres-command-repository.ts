...
const map = (r: Row): Command => ({
  id:r.id, managedDeviceId:r.managed_device_id, adminId:r.admin_id, type:r.type, version:r.version,
  status:r.status, payload:r.payload, correlationId:r.correlation_id, idempotencyKey:r.idempotency_key,
  createdAt:r.created_at, expiresAt:r.expires_at, deliveryAt:r.delivery_at, acknowledgedAt:r.acknowledged_at,
  startedAt:r.started_at, completedAt:r.completed_at, cancelledAt:r.cancelled_at,
...
export class PostgresCommandRepository extends PostgresRepository implements CommandRepository {
  ...
  async cancelOwned(id:string,adminId:string,now:Date):Promise<Command>{
    return this.transition({id,from:'CREATED',to:'CANCELLED',actorType:'ADMIN',actorId:adminId,now,correlationId:null});
  }

  async transition(input:{
    id:string; from:CommandStatus; to:CommandStatus; actorType:CommandActorType; actorId:string|null; now:Date;
    correlationId:string|null; failureCode?:string|null; errorCategory?:string|null; resultCode?:string|null;
    ...
    return this.transaction(async client=>{
      ...
      if(input.now.getTime()>=current.expires_at.getTime() && !['EXPIRED','CANCELLED'].includes(input.to)){
        ...
        await client.query('INSERT INTO command_events (id,command_id,from_status,to_status,actor_type,actor_id,correlation_id,occurred_at) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)',[crypto.randomUUID(),input.id,current.status,'EXPIRED','SYSTEM',null,current.correlation_id,input.now]);
        throw new PersistenceError('INVALID_STATE','Command expired.');
      }
      const fields:string[]=['status=$2'];
      const values:unknown[]=[input.id,input.to];
      const set=(sql:string,v:unknown)=>{fields.push(sql);values.push(v);};
      if(input.to==='DELIVERING') set('delivery_at=COALESCE(delivery_at,$'+(values.length+1)+')',input.now);
      if(input.to==='ACKNOWLEDGED') set('acknowledged_at=$'+(values.length+1),input.now);
      if(input.to==='RUNNING') set('started_at=COALESCE(started_at,$'+(values.length+1)+')',input.now);
      if(['SUCCEEDED','FAILED','EXPIRED','CANCELLED','REJECTED'].includes(input.to)) set('completed_at=$'+(values.length+1),input.now);
      if(input.to==='CANCELLED') set('cancelled_at=$'+(values.length+1),input.now);
      if(input.failureCode!==undefined) set('failure_code=$'+(values.length+1),input.failureCode);
      if(input.errorCategory!==undefined) set('error_category=$'+(values.length+1),input.errorCategory);
      if(input.resultCode!==undefined) set('result_code=$'+(values.length+1),input.resultCode);
      if(input.resultMetadata!==undefined) set('result_metadata=$'+(values.length+1),input.resultMetadata===null?null:JSON.stringify(input.resultMetadata));
      values.push(input.id);
      const numbered=fields.map((f,i)=>f.replace(/\$(\d+)/g,(_,n)=>'$'+(Number(n)+1)));
      const sql='UPDATE commands SET '+numbered.join(', ')+' WHERE id=$'+values.length+' RETURNING '+columns;
      const updated=await client.query<Row>(sql,values);
      const command=map(updated.rows[0]!);
      await client.query('INSERT INTO command_events (id,command_id,from_status,to_status,actor_type,actor_id,correlation_id,occurred_at) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)',[crypto.randomUUID(),input.id,current.status,input.to,input.actorType,input.actorId,input.correlationId??current.correlation_id,input.now]);
      return command;
    });
  }
  ...
  private async recordEvent(commandId:string,fromStatus:CommandStatus|null,toStatus:CommandStatus,actorType:CommandActorType,actorId:string|null,correlationId:string|null):Promise<void>{
    await this.query('INSERT INTO command_events (id,command_id,from_status,to_status,actor_type,actor_id,correlation_id) VALUES ($1,$2,$3,$4,$5,$6,$7)',[crypto.randomUUID(),commandId,fromStatus,toStatus,actorType,actorId,correlationId]);
  }
}
