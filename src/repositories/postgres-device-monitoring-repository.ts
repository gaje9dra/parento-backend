...
interface StateRow extends QueryResultRow {
  ...
  battery_charging_state: string | null;
...
const mapState = (row: StateRow): DeviceMonitoringState => {
  if (row.observed_at === null || row.received_at === null)
    throw new PersistenceError(
      'NOT_FOUND',
      'Monitoring state was not reported.',
    );
  return {
    ...
    batteryChargingState:
      row.battery_charging_state as DeviceMonitoringState['batteryChargingState'],
    batteryStatus: row.battery_status as DeviceMonitoringState['batteryStatus'],
    networkState: row.network_state as DeviceMonitoringState['networkState'],
    storageTotalBytes:
      row.storage_total_bytes === null ? null : Number(row.storage_total_bytes),
    storageAvailableBytes:
      row.storage_available_bytes === null
        ? null
        : Number(row.storage_available_bytes),
    storageUsedBytes:
      row.storage_used_bytes === null ? null : Number(row.storage_used_bytes),
    memoryTotalBytes:
      row.memory_total_bytes === null ? null : Number(row.memory_total_bytes),
    memoryAvailableBytes:
      row.memory_available_bytes === null
        ? null
        : Number(row.memory_available_bytes),
    memoryLow: row.memory_low,
    managementMode:
      row.management_mode as DeviceMonitoringState['managementMode'],
  ...
  implements DeviceMonitoringRepository
{
  ...
  async findCurrent(
    ...
    const r = await this.query<StateRow>(
      'SELECT ' +
        selectColumns +
    ...
    return r.rows[0] === undefined || r.rows[0].observed_at === null
      ? null
      : mapState(r.rows[0]);
  }
  async upsertIfNewer(
    ...
    try {
      return await this.transaction(async (client) => {
        ...
        const update = names
          .filter((n) => n !== 'observed_at' && n !== 'schema_version')
          .map((n) => n + '=EXCLUDED.' + n)
          .concat([
        ...
        const r = await client.query(
          'INSERT INTO device_monitoring_state (managed_device_id,' +
            ...
            update.join(',') +
            ' WHERE EXCLUDED.observed_at > device_monitoring_state.observed_at RETURNING managed_device_id',
          [snapshot.managedDeviceId, ...values],
        );
        if (r.rowCount !== 1) return 'ignored';
        await client.query(
          'UPDATE managed_devices SET last_seen_at=$2,updated_at=NOW() WHERE id=$1 AND (last_seen_at IS NULL OR last_seen_at<$2)',
  ...
  async listForAdmin(
    ...
    const cursor = page.cursor ? cursorDecode(page.cursor) : null;
    const where = cursor
  ...
  async findForAdmin(
    ...
    if (r.rows[0] === undefined) return null;
    const row = r.rows[0];
    return {
      state: row.observed_at === null ? null : mapState(row),
      enrollmentStatus: row.enrollment_status,
