  ...
  implements ManagedDeviceRepository
{
  ...
  async findById(id: string): Promise<ManagedDevice | null> {
    const result = await this.query<DeviceRow>(
      'SELECT id, admin_id, name, platform, enrollment_status, operational_status, created_at, updated_at, last_seen_at FROM managed_devices WHERE id = $1',
      [id],
    );
  ...
  async listByAdminId(adminId: string): Promise<ManagedDevice[]> {
    const result = await this.query<DeviceRow>(
      'SELECT id, admin_id, name, platform, enrollment_status, operational_status, created_at, updated_at, last_seen_at FROM managed_devices WHERE admin_id = $1 ORDER BY created_at DESC',
      [adminId],
    );
