...
export interface DeviceConnectionSessionRepository extends Repository {
  create(input: { id: string; managedDeviceId: string; sessionTokenHash: string; expiresAt: Date }): Promise<DeviceConnectionSession>;
  findByTokenHash(tokenHash: string): Promise<DeviceConnectionSession | null>;
  touchConnected(id: string, now: Date, expiresAt: Date): Promise<DeviceConnectionSession | null>;
  disconnect(id: string, now: Date, state?: DeviceConnectionState): Promise<DeviceConnectionSession | null>;
