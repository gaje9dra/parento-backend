...
const mapPolicy = (r: PolicyRow, rules: RuleRow[]): ApplicationPolicy => ({
  ...
  rules: rules
    .filter((rule) => rule.policy_id === r.id)
    .map((rule) => ({
      id: rule.id,
  ...
  implements ApplicationManagementRepository
{
  ...
  async replaceInventory(input: {
    ...
    try {
      ...
      throw mapPostgresPersistenceError(
        error,
        'Unable to synchronize application inventory.',
      );
    }
  }
  ...
  async listInventory(
    ...
    const result = cursorValue
      ? await this.query<InventoryRow>(
          'SELECT ' +
            inventoryColumns +
            ' FROM application_inventory WHERE managed_device_id=$1 AND package_name>$2 ORDER BY package_name ASC LIMIT $3',
          [managedDeviceId, cursorValue.packageName, limit + 1],
        )
      : await this.query<InventoryRow>(
          'SELECT ' +
            inventoryColumns +
            ' FROM application_inventory WHERE managed_device_id=$1 ORDER BY package_name ASC LIMIT $2',
          [managedDeviceId, limit + 1],
        );
    const rows =
      result.rows.length > limit ? result.rows.slice(0, limit) : result.rows;
    const nextCursor =
      result.rows.length > limit
        ? Buffer.from(
            JSON.stringify({
              packageName: rows[rows.length - 1]!.package_name,
            }),
          ).toString('base64url')
        : null;
  ...
  async findInventoryItem(
    ...
    const result = await this.query<InventoryRow>(
      'SELECT ' +
        inventoryColumns +
        ' FROM application_inventory WHERE managed_device_id=$1 AND package_name=$2',
    ...
    try {
      return await this.transaction(async (client) => {
        const result = await client.query<PolicyRow>(
          'INSERT INTO application_policies (id,admin_id,name,description,created_by,updated_by) VALUES ($1,$2,$3,$4,$5,$5) RETURNING ' +
            policyColumns,
          [
            input.id,
            input.adminId,
        ...
        return mapPolicy(
          ...
          input.rules.map((rule) => ({
            ...
            action: rule.action as ApplicationAction,
          })),
        );
      });
    } catch (error) {
      throw mapPostgresPersistenceError(
        error,
        'Unable to create application policy.',
      );
    }
  }
    ...
    try {
      return await this.transaction(async (client) => {
        ...
        const updated = await client.query<PolicyRow>(
          'UPDATE application_policies SET name=$2,description=$3,version=version+1,updated_by=$4,updated_at=NOW() WHERE id=$1 RETURNING ' +
            policyColumns,
      ...
      throw mapPostgresPersistenceError(
        error,
        'Unable to update application policy.',
      );
    }
  }
  ...
  async disablePolicy(
    ...
    try {
      return await this.transaction(async (client) => {
        const result = await client.query<PolicyRow>(
          "UPDATE application_policies SET status='DISABLED',version=version+1,updated_by=$3,updated_at=NOW() WHERE id=$1 AND admin_id=$2 RETURNING " +
            policyColumns,
      ...
      throw mapPostgresPersistenceError(
        error,
        'Unable to disable application policy.',
      );
    }
  }
  ...
  async listPolicies(adminId: string): Promise<ApplicationPolicy[]> {
    const policies = await this.query<PolicyRow>(
      'SELECT ' +
        policyColumns +
        ' FROM application_policies WHERE admin_id=$1 ORDER BY updated_at DESC,id DESC',
      [adminId],
    );
  ...
  async assignPolicy(input: {
    ...
    try {
      return await this.transaction(async (client) => {
        ...
        if (!device.rows[0])
          throw new PersistenceError('NOT_FOUND', 'Managed device not found.');
        const policy = await client.query<PolicyRow>(
          'SELECT ' +
            policyColumns +
            " FROM application_policies WHERE id=$1 AND admin_id=$2 AND status='ACTIVE' FOR UPDATE",
          [input.policyId, input.assignedBy],
        );
        if (!policy.rows[0] || policy.rows[0].version !== input.policyVersion)
          throw new PersistenceError(
            'INVALID_STATE',
            'Application policy is stale or unavailable.',
          );
      ...
      throw mapPostgresPersistenceError(
        error,
        'Unable to assign application policy.',
      );
    }
  }
  ...
  async getEnforcementState(
    ...
    const result = await this.query<EnforcementRow>(
      'SELECT e.' +
        enforcementColumns.replaceAll(',', ',e.') +
  ...
  async recordEnforcement(input: {
    ...
    try {
      const result = await this.query<EnforcementRow>(
        'UPDATE application_enforcement_state SET reported_policy_id=$2,reported_policy_version=$3,status=$4,synchronization_required=CASE WHEN desired_policy_id=$2 AND desired_policy_version=$3 THEN FALSE ELSE TRUE END,last_reported_at=$5,failure_code=$6,updated_at=$5 WHERE managed_device_id=$1 RETURNING ' +
          enforcementColumns,
        [
          input.managedDeviceId,
          input.reportedPolicyId,
          input.reportedPolicyVersion,
          input.status,
          input.reportedAt,
          input.failureCode,
        ],
      ...
      throw mapPostgresPersistenceError(
        error,
        'Unable to record application enforcement state.',
      );
    }
  }
}
