...
## Phase 2.1 — Backend Database Foundation & Persistence Architecture

Phase 2.1 establishes the PostgreSQL persistence foundation without implementing authentication, enrollment, devices, policies, commands, monitoring, or other future business entities.

### Database technology

The backend uses PostgreSQL through the `pg` Node.js client. No heavyweight ORM was introduced because Phase 1 had not already selected one and the current phase only requires a production-capable relational query layer.
...
`src/repositories/postgres-repository.ts` provides the reusable repository boundary for later business repositories.

### Configuration

Database settings are centralized in `src/config/env.ts`:
- `DATABASE_URL`
- `DATABASE_POOL_MAX`
...
Production requires `DATABASE_URL`. Development/test may intentionally omit it for HTTP/configuration-only execution.

No production credentials or connection strings are committed.

### Connection lifecycle

When configured, one process-wide PostgreSQL pool is created.

Startup connects before the HTTP server begins listening. Runtime operations reuse the pool. Transactions use a checked-out pool client and guarantee rollback/release on failure. Graceful shutdown closes the pool.

### Migrations

Version-controlled SQL migrations live under `migrations/`.

The runner owns the `schema_migrations` table and supports:
...
The initial Phase 2.1 migration is `migrations/0001_phase_2_1_baseline.sql`.

It intentionally creates no Parento business entities. It establishes only the migration baseline required for later phases.

`db:reset` is explicitly blocked in production.

### Readiness

Health remains lightweight: `GET /api/v1/health`.

Readiness now distinguishes application state from configured PostgreSQL state: `GET /api/v1/ready`.
...
A configured but unavailable database returns HTTP 503. Readiness uses only a lightweight `SELECT 1`.

### Testing

Database integration tests are in `tests/database.integration.test.ts`.

They cover PostgreSQL connectivity, clean migration application, migration status, deterministic repeated migrations, and transaction rollback.

When `DATABASE_URL` is absent, these integration tests are skipped. CI provisions a disposable PostgreSQL 17 service and executes the database migration/integration suite.

### CI

The verification workflow now provisions PostgreSQL for tests and runs:
...
`npm install` is currently used in CI because the repository dependency lockfile predates the Phase 2.1 PostgreSQL dependency and has not been regenerated in this environment. The lockfile must be regenerated and committed before relying on `npm ci` again.

### Documentation

Phase 2.1 architecture details are documented in `docs/phase-2.1-database.md`.

### Cross-repository boundary

Only `gaje9dra/parento-backend` was modified.

No changes were made to:
- `gaje9dra/parento-admin`
- `gaje9dra/parento-managed`

Future integrations remain documentation-level requirements only.
